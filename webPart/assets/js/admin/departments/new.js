$( document ).ready(function() {
    $('*[data-action="new"]').on('click', function() {
        ...
        $.post( SITE + "api/admin/departments/create", { name: name.val(), groups: groups.val(), sv_group: supervisor_group.val(), training: training.val(), type: type.val() }, ( data ) => {
            if(data.status == 'success') {
                Notiflix.Notify.Success(data.message)
                setTimeout(() => { window.location = SITE + 'admin/departments' }, 1500)
            } else {
